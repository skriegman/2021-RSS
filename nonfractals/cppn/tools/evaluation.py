...
def body_func(data, n_recursions, aggregation1=None, aggregation2=None):
    ...
    if aggregation1 is None:
        ...
        size0 = block
        size1 = blockify(aggregation1, size0)
        size2 = blockify(aggregation2, size1)

        sizes = [size0, size1, size2]
        return sizes[n_recursions]
        ...
        for n, ind in enumerate(pop[:N]):
            ...
            # new designs are evaluated with teammates from the entire population (new and old).
                ...
                for name, details in ind.genotype.to_phenotype_mapping.items():
                    ...
                    state = details["state"]
                    if name == "Data":
                        body = body_func(state, r_num, agg, agg_of_aggs)
